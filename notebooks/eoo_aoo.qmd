...
packages <- c("googledrive", "lwgeom", "ggplot" , "ggspatial", "sf", "devtools")
...
data_file_id <- "1Wc50TQSbPV2xSl10nmFoUvzhiQrzBhqn" # Replace with the actual file ID of the CSV file

# Define workspace path
datafolder_path <- file.path(workspace)

# Create a local path for the CSV file
csv_file_path <- file.path(datafolder_path, "raster_preparation.csv")

# Function to download a CSV file with progress messages
download_csv_file <- function(file_id, file_path) {
  cat("Downloading CSV file...\n")
  drive_download(as_id(file_id), path = file_path, overwrite = TRUE)
  cat("Downloaded CSV file to:", file_path, "\n")
}
...
cat("Starting to download the CSV file...\n")
download_csv_file(data_file_id, csv_file_path)

cat("CSV file is ready for use at:", csv_file_path, "\n")
