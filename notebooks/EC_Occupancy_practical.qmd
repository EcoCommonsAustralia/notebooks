...
- Determine the structure of the **detection history matrices** you will create: 
  ...
  - First, you must determine the _maximum duration_ of the matrix (i.e., number of columns), and its recommended to not exceed 100 days to satisfy the population closure assumption of the model. 
  
  - Next, you must determine the _sampling window duration_ which is the number of days that fit into one column in the detection history matrix. Larger intervals (*e.g.*, 7-10 days) are typically better for rare species with very low detections, while shorter values (*e.g.*, 2-5 days) are better for frequently detected species. 
...
We successfully made the occupancy umf for each species. Lets inspect the umf object for a single species, *Felis catus*. 

```{r occupancy_result_inspect_3}
## inspect one UMF 
# cats occupancy model
# umf = umf_list$`Felis catus`
umf = umf_list$`Orthonyx spaldingii`
...
This null occupancy model estimates that chowchilla occupy about X% of sites, with a per-occasion detection probability of roughly Y%, indicating imperfect detection. The low detection probability explains why many occupied sites have few or no detections. This model provides a baseline against which more complex models with detection and environmental covariates can be compared.
...
The `WildObsR::matrix_generator()` function has already standardized numeric covariates to have a mean of zero and a standard deviation of 1 to ensure comparable model outputs. 
