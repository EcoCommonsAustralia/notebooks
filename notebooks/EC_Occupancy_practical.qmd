...
There are two notebooks about Single-Species Single-Season Occupancy Model, one is conceptual, and another is practical. This one is the **practical** one which gives a brief introduction to the concepts of Single-Species Single-Season Occupancy Model with some simulated data.
...
This practical notebook demonstrates how to build a **single-species, single-season occupancy model** using camera-trap data curated and standardised through the **Wildlife Observatory of Australia (WildObs)** platform. WildObs processes raw camera-trap images using artificial intelligence (e.g., MegaDetector, SpeciesNet), applies the CamtrapDP data standard, and provides harmonised deployments, observations, and covariates that are ready for ecological analysis.
...
-   download and inspect WildObs camera-trap projects

-   prepare site-level and observation-level covariates

-   spatially resample deployments into biologically meaningful sampling units

-   construct detection history matrices using matrix_generator()

-   build the unmarkedFrameOccu (UMF) object

-   fit occupancy models, compare candidate models, and interpret the results
...
|                                                                                                                       |                                   |                                                                   |
|                                                                                                                       |                                   | 1.5 Abundance model and mapping                                   |
+-----------------------------------------------------------------------------------------------------------------------+-----------------------------------+-------------------------------------------------------------------+
...
## load libs
# WildObsR    for bespoke data wrangling function
# frictionless    for importing data packages and extracting resources 
# unmarked      for running frequentist occupancy/abundance models 
...
Since WildObsR is frequently updated, please use the below function to keep your WildObsR r package the most updated version.
...
Because of the logistical advantages of camera traps - such as low labour intensity, low cost, and long temporol coverage - they are widely used in wildlife research and monitoring, generating datasets that can be applied in occupancy models (Anderson et al., 2025; Bruce et al., 2025).
...
### 1.2.1 Atlas of Living Australia
...
### 1.3.1 Query WildObs MongoDB for Relevant Project IDs
...
dp_list <- wildobs_dp_download(
  ...
  media = FALSE,              # We don't need media files for this notebook
  metadata_only = FALSE
)
```

**dp_list** is list object containing **Camera Trap Data Package (Camrap DP)** which is a community-developed data exchange format for camera trap data.
...
**Step 2:** Just like in our data simulation in [Single-Species Single-Season Occupancy Model - **Conceptual**](https://github.com/ecocommonsaustralia/notebooks/blob/main/notebooks/EC_Occupancy_conceptual.qmd) ,

we must extract the three resources:
...
# Clean up
rm(covariates_i, deployments_i, observations_i, dp, i)
...
The *observations* table contains event-level records detected by each camera, including timestamps, media identifiers, and links back to their deployment. Each row represents either a blank trigger or an animal detection, with associated metadata such as species identification, life stage, sex, behaviour, bounding-box coordinates, classification method, and timing variables (*deltaTime_event*, *deltaTime_obs*). The first six rows show a mix of blank events and a detection of *Megapodius reinwardt* within the CFRAG_01 deployment.
...
The *covariates* table provides environmental and landscape attributes associated with each camera deployment. It includes spatial location (latitude/longitude), deployment period, and group labels, along with a wide range of ecological covariates summarised at multiple spatial scales (point, 1 km², 3 km², 5 km², 10 km²). These covariates capture habitat condition (e.g., FLII), human pressure (human footprint), topography (altitude), climate variables (temperature, precipitation), vegetation indices (NDVI/EVI if included), nighttime lights, and other environmental descriptors. The first six rows show covariate profiles for the CFRAG deployments in the Eacham–Curtain Fig project.
